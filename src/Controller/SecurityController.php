...
class SecurityController extends AbstractController
{
    ...
    public function forgottenPassword(Request $request, UserRepository $userRepository, SecurityTokenManager $securityTokenManager):Response
    {
        ...
        { 
            ...
            // TODO  $securityTokenManager->requirePasswordResetConfirmationFrom($user)
        }
    ...
    public function newCode2fa(Request $request, UserRepository $userRepository, SecurityTokenManager $securityTokenManager): Response
    {
        $user = $userRepository->findOneBy(['email' => $request->getSession()->get(AppAuthenticator::LAST_USERNAME)]);
        if($user)
        {
            $securityTokenManager->requireCode2faFrom($user);
    ...
    public function newConfirmationEmail(Request $request, UserRepository $userRepository, SecurityTokenManager $securityTokenManager): Response
    {
        $user = $userRepository->findOneBy(['email' => $request->getSession()->get(AppAuthenticator::LAST_USERNAME)]);
        if($user)
        {
